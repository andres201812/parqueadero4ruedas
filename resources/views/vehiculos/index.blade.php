							...
							<table id="example1" class="table table-bordered table-hover" style="width:100%">
                                ...
                                <tbody>
									...
									<tr>
										...
										<td>{{ $vehiculo->propietario->numerodocumento }}</td>
										<td>{{ $vehiculo->tipovehiculo->nombre }}</td>
										<td>{{ $vehiculo->marca->nombre }}</td>
										<td>{{ $vehiculo->placa }}</td>
										<td>
											<a href="{{ route('vehiculos.edit',$vehiculo->id) }}" class="btn btn-info btn-sm" title="Editar"><i class="fas fa-pencil-alt"></i></a>
											<form class="d-inline delete-form" action="{{ route('vehiculos.destroy', $vehiculo->id) }}"  method="POST">
												@csrf
												@method('DELETE')
